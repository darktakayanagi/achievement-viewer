...
│   └── 928960/                        # Godfall
│       ├── achievements.json          # Your achievement data (only one "Your achievement data" file is required)
│       ├── Epic.platform              # Platform indicator empty file (displays the text before ".platform" on the game card below AppID)
│       ├── blacklist                  # Skipped achievements to display. You have to use achievement API names in every lines! (Useful in cases like with Godfall where Steam version has more achievements than Epic)
│       └── game-info.json             # Auto-generated by workflow (optional)
